//ifElse.h
//IfElse example that uses a student Class that stores a student name and Average
#include <iostream>
...
public:
	...
	Student(std::string studentName, int studentAverage) 
		...
		void setAverage(int studentAverage) {
			//validate that studentAverage is >0 and <=100; otherwise, keep data member average's
			//current value
			if(studentAverage > 0) {
...
private:
	std::string name;
	int average{0}; //initializa average to 0
}; // end class Student