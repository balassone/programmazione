...
LinkedList<NODETYPE>& LinkedList<NODETYPE>::operator=(const LinkedList<NODETYPE> &lhs) {
	if (&lhs != this) {	//checks if the current list is different from the lhs
        Node<NODETYPE> *tmp = head; 	// stores the head of the current list

        while (tmp->next != nullptr) { 	 // deleting the element from the old list
            head = head -> next;
            delete tmp;
            tmp = head;
        }
        tmp = lhs.head;	//going to the head of the other list
...
template<typename NODETYPE>
void LinkedList<NODETYPE>::add(const NODETYPE& e, int i){
	if(i == 0){addFront(e);}
