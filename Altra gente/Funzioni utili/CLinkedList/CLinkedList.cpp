...
template <typename T>
CircleList<T> CircleList<T>::reverseCopy(){
	CircleList<T> C;
	C.cursor = new CNode<T>;
	C.cursor->elem = cursor->next->elem;
	C.cursor->next = C.cursor;
	CNode<T> * v = cursor->next->next;
	do{
		C.add(v->elem);
		v = v->next;
	}while(v != cursor->next);
