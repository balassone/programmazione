...
LinkedList<NODETYPE>& LinkedList<NODETYPE>::operator=(const LinkedList<NODETYPE> &lhs) {
	if (&lhs != this) {	//controlla se la lista attuale e la lista in ingresso non siano le stesse
        Node<NODETYPE> *tmp = head; 	// conserva la testa della lista

        while (tmp->next != nullptr) { 	 // cancella gli elementi precedenti nella lista
            head = head -> next;
            delete tmp;
            tmp = head;
        }
        tmp = lhs.head;	//posizionamento nella testa dell'altra lista

        while (tmp != nullptr) {	//dando in ingresso gli elementi dell'altra lista
            addBack(tmp->elem);		//permette di stampare gli elementi della lista dal retro
            tmp = tmp -> next;
        }
    }
...
void LinkedList<NODETYPE>::add(const NODETYPE& e){ // aggiunta di un nuovo elemento
    ...
    if(empty()){		// controlla se la lista è vuota
        ...
        NodoP * v = head;			//tiene traccia del nodo corrente
        NodoP * p = nullptr;		//tiene traccia del nodo precedente
        while (v != nullptr){		
